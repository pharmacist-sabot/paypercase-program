...
<div class="flex-col h-full" style="gap:16px; padding:16px">
    ...
    <div class="group-box">
        ...
        <div style="padding-top:12px">
            <div class="flex items-center gap-12 mb-12" style="flex-wrap:wrap">
                ...
                <input type="date" v-model="dateFrom" style="width:150px" />
                <span class="nowrap">วันที่สิ้นสุด :</span>
                <input type="date" v-model="dateTo" style="width:150px" />
            </div>

            <div v-if="rangeInfo" class="alert" :class="rangeInfo.type === 'info' ? 'alert-info' : 'alert-warning'"
                style="margin-bottom:10px">
                {{ rangeInfo.text }}
            </div>

            <div class="flex items-center gap-8">
                <button class="btn btn-secondary" @click="previewRange">🔍 ดูรายการที่จะลบ</button>
                <button class="btn btn-danger" :disabled="previewRows.length === 0" @click="deleteRange">
                    🗑️ ลบข้อมูลในช่วงวันที่เลือก
                </button>
            </div>

            <!-- Preview table -->
            <div v-if="previewRows.length > 0" class="table-wrapper" style="max-height:200px;margin-top:12px">
                <table class="data-table">
                    ...
                    <tbody>
                        <tr v-for="(r, i) in previewRows" :key="r.id">
                            ...
                            <td>{{ r.therapist }}</td>
                            <td style="text-align:right">{{ r.payout_amount.toFixed(2) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <div class="group-box flex-col" style="flex:1;min-height:0">
        ...
        <div class="flex items-center gap-8 mb-8" style="padding-top:12px">
            <button class="btn btn-secondary btn-sm" @click="loadStats">🔄 รีเฟรช</button>
        </div>
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th class="sortable" @click="sortStats('total_revenue')">
                            รายได้รวม (฿) {{ sortIcon('total_revenue') }}
                        </th>
                        <th class="sortable" @click="sortStats('total_payout')">
                ...
                <tbody>
                    <tr v-for="s in sortedStats" :key="s.month">
                        ...
                        <td style="text-align:right">{{ s.total_revenue.toFixed(2) }}</td>
                        <td style="text-align:right;font-weight:bold;color:var(--highlight)">
                            {{ s.total_payout.toFixed(2) }}
                        </td>
                    </tr>
                    <tr v-if="stats.length === 0">
                        <td colspan="4" style="text-align:center;padding:24px;color:var(--text-gray)">
                            ไม่มีข้อมูล
                        </td>
                    </tr>
                </tbody>
...
async function loadStats() {
    try { stats.value = await cmd.getMonthlyStats() }
    catch (e: any) { error.value = String(e) }
}

async function previewRange() {
    error.value = ''; rangeInfo.value = null
    try {
        previewRows.value = await cmd.previewDeleteRange(dateFrom.value, dateTo.value)
        rangeInfo.value = previewRows.value.length > 0
            ? { type: 'warning', text: `พบ ${previewRows.value.length} รายการที่จะถูกลบในช่วงวันที่นี้` }
            : { type: 'info', text: 'ไม่พบข้อมูลในช่วงวันที่ที่เลือก' }
    } catch (e: any) { error.value = String(e) }
}

async function deleteRange() {
    if (!confirm(`ต้องการลบข้อมูล ${previewRows.value.length} รายการในช่วงวันที่ ${dateFrom.value} ถึง ${dateTo.value} ใช่หรือไม่?\n\nการกระทำนี้ไม่สามารถย้อนกลับได้`))
        return
    error.value = ''
    try {
